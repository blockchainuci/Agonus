...
class AgentMemory:
    ...
    The AgentMemory class manages three distinct memory systems:
        1️⃣ Short-term memory — In-memory list of Trade objects for the current tournament.
        2️⃣ Long-term memory — Persistent database of all trades and performance statistics across tournaments.
        3️⃣ Vector memory — Semantic recall system for embedding-based similarity search of significant trades.

    This class provides the memory interface used by the BaseAgent
    for recording, retrieving, and reasoning over past experiences.
    All complex logic (database writes, embeddings, etc.) will be implemented later.
    ...
    vector_db : Any
        Handle to the vector database client (e.g., Pinecone, Chroma) for semantic recall.
    max_short_term : int
        Optional maximum number of trades to keep in memory during a tournament (used for pruning).
    performance_metrics : Dict[str, float]
        Cached high-level performance statistics for quick reference (win_rate, avg_profit, etc.).
    """
    ...
    def __init__(
        ...
        """
        Initialize a new AgentMemory instance.

        Parameters
        ----------
        agent_id : str | int
            Unique ID assigned to this agent instance.
        long_term_db : Any, optional
            Database client for durable storage and retrieval of trade history.
        vector_db : Any, optional
            Vector index client for semantic recall and embedding search.
        max_short_term : int, optional
            Maximum number of trades to keep in memory during a tournament.
        """
