...
class AgentResearchArtifact(Base):
    """Shared research cache keyed by crypto_token. One row per token, UPSERT on refresh."""

    __tablename__ = "agent_research_artifact"

    __table_args__ = (
        Index("ix_research_token_updated", "crypto_token", "updated_at"),
    )
    ...
    recency: Mapped[Optional[str]] = mapped_column(String, default=None)  # "1d", "7d", "30d", "365d"
    ...
    agent_opinion: Mapped[Optional[str]] = mapped_column(
        SQLEnum(OpinionEnum, native_enum=False), default=None, nullable=True
    )
